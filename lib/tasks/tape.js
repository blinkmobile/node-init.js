...
function npmInstall(cwd) {
  return execa('npm', ['install', '--save-dev', 'tape', 'tap', 'faucet'], {
    cwd
  });
}
  ...
  return updateJsonFile(
    ...
    pkg => {
      addScript(
        pkg,
        'tape',
        "find . -path ./node_modules -prune -o -type f -name '*.test.js' -print0 | xargs -0 tape | faucet"
      );
      addScript(pkg, 'test', 'npm run tape'); // `npm test` runs tape
...
function isNeeded({ cwd } /* : TaskOptions */) {
  return getPkg(cwd).then(
    pkg => !hasTestFramework(pkg) && isTestScriptUnconfigured(pkg)
  );
}
